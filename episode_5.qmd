...
::: {.grid}
::: {.g-col-6}
![](theme_images/over-rep_theme.png){width=70%}
:::
...
-   **Pathway topology**: Pathway topology analysis takes a higher level approach, compiling information on up- and down-regulation at the level of the pathway rather than the level of the gene (e.g., [KEGG](https://www.genome.jp/kegg/pathway.html), [Reactome](https://reactome.org/), [PANTHER](https://pantherdb.org/), [SPIA](https://bioconductor.org/packages/release/bioc/html/SPIA.html))

Today we will show an example of using GOseq -- an over-representation analysis method specifically for RNAseq data. 
...
We will be using Gene Ontology (GO) annotations to functionally classify our genes. GO terms can be divided into three aspects -- Molecular Function (MF), Cellular Component (CC), and Biological Process (BP). Read more about [GO terms here.](https://geneontology.org/docs/ontology-documentation/)
...
# Help! My tt.RData does not exist/did not load in:

In the previous episode we created and saved this tt.RData, but we have stored a copy of it on the GitHub repository that you can load in directly if you don't have it:

```{r}
#| eval: false

load(url("https://raw.githubusercontent.com/otagobioinformaticsspringschool/RNA-seq-data-analysis-workflow/main/tt.RData"))
```
...
# How to include the logFC cutoff too:
...
Here we showed an example of how to run GOseq using annotation data pulled from the AnnotationDbi package and the yeast specific Bioconductor organism annotation package org.Sc.sgd.db. But what if you don't have a supported organism, can you still run GOseq? 

Yes you can!

You will need:  
- A file of all your gene IDs (i.e., all your background genes) in one column, matched to their gene lengths in another column.  
- A file of your gene IDs (i.e., all your background genes in one column, matched to their GO terms in another column.  
- A list of your DE genes IDs.  

You can generate these yourself, pulling information from the genome, or *de novo* assembled transcriptome data, and if necessary, performing GO annotation manually using software such as BLAST2GO and PFAM2GO or InterPro. 
...
This approach has the advantage of not relying on our somewhat arbitrary cutoff thresholds (*e.g.,* a gene with a p-value 0.0501 probably should still contribute to what we think is happening to a pathway). Simply put, GOseq is threshold-based and GSEA is ranking-based.

In particular we recommend The Carpentries incubator workshop RNA-seq analysis with Bioconductor [Gene set enrichment analysis section.](https://carpentries-incubator.github.io/bioc-rnaseq/instructor/07-gene-set-analysis.html) 
...
In any situation where there are multiple tools, methods, or decision points (*e.g.,* thresholds, removal of outliers *etc.,*) it is recommended that you complete the analyses and compare the outcomes. 

It is possible to run GOSeq *without* the length correction implemention (shown below) and compare the over-represented categories. If the difference is large, then you should be cautious when interpreting GSEA results or the output of clusterProfiler. 
